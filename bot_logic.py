...
بدون أي حاجة لتليجرام. تُستخدم مباشرة من واجهة التطبيق (main.py).
...
TMDB_API_KEY = "267b5b6916bb1e7f59f613ed78f54c12"
TMDB_API_KEY_2 = "53aebbda42312a223dae9560881c3905"
SUBSOURCE_API_KEY = "sk_672398fa8bdd191d48e598c408d786a6afa061e336236fc5c40b332678f67100"
SUBSOURCE_API_KEY_2 = "sk_ea2e48cfd708dc9937a770ed36f654fc3bc4d3f019822f761e87293736e6c7ec"
...
def format_movie_info(item: dict) -> str:
    """يرجّع نص منسّق فيه العنوان والسنة والنوع والتقييم والوصف."""
...
def _subsource_request(method: str, url: str, **kwargs):
    """يجرب مفتاح SubSource الأول، وإذا رجع 401 يجرب الثاني."""
    timeout = kwargs.pop("timeout", 15)
    keys = _subsource_keys()
    if not keys:
        raise SubSourceAuthError("لا يوجد أي مفتاح SubSource مُعبّأ")

    last_auth_error = None
    for key in keys:
        resp = requests.request(method, url, headers=_subsource_headers(key), timeout=timeout, **kwargs)
...
def subsource_download(subtitle_id: int) -> bytes:
    resp = _subsource_request("GET", f"{SUBSOURCE_BASE}/subtitles/{subtitle_id}/download", timeout=30)
    return resp.content
...
def format_subtitle_label(sub: dict, index: int) -> str:
    """يرجّع نص قصير لعرضه كزر لكل ترجمة بالنتائج."""
