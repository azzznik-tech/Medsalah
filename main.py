...
def _get_downloads_dir():
    """تنزيلات العام أولاً، أو تخزين التطبيق."""
    candidates = [
...
class MovieSubsApp(App):
    def build(self):
        self.title = "مبحث الأفلام والترجمات"
        if IMPORT_ERROR:
    ...
    def _error_screen(self, message):
        ...
        root.add_widget(Label(text="حدث خطأ — هذه الشاشة صورة:", size_hint=(1, 0.08)))
        error_label = Label(
            text=message, size_hint_y=None, halign="left",
    ...
    def _build_ui(self):
        ...
        self.status_label = Label(
            text="اكتب اسم فيلم وابدأ البحث", size_hint=(1, 0.06),
            font_size="13sp"
        )
        ...
        root.add_widget(Label(text="الترجمات المتوفرة:", size_hint=(1, 0.05),
                              font_size="13sp"))
        self.subs_list = BoxLayout(orientation="vertical", size_hint_y=None,
                                   spacing=6)
    ...
    def _search_thread(self, query):
        ...
        if not movies:
            self._set_status("لم أجد هذا العمل في قاعدة الترجمات.")
            return
        ...
        try:
            subtitles = bot_logic.subsource_get_subtitles(
                movie_id, language="arabic"
            )
        except Exception as e:
        ...
        self._set_status(f"وجدت {len(subtitles)} ترجمة — اختر واحدة من الأسفل")
    ...
    def on_pick_subtitle(self, index):
        ...
        threading.Thread(target=self._download_thread, args=(sub_id,),
                         daemon=True).start()
    ...
    def _download_thread(self, sub_id):
        try:
            ...
            return
        if not raw:
            self._set_status("أعاد التحميل بيانات فارغة.")
            return
        srt_data, filename = bot_logic.extract_srt(
            raw, fallback_name=self._current_title or "subtitle"
        )
